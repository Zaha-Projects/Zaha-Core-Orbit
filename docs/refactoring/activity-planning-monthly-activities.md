...
- تعذر تشغيل Laravel tests حاليًا لأن `vendor/autoload.php` غير موجود. نجاح lint ليس بديلًا عن الاختبارات.
...
- [ ] تثبيت `vendor` من lock file دون تحديث dependencies.
...
- [ ] تشغيل tests بعد توفر vendor؛ لا تعتبر المرحلة مكتملة سلوكيًا قبل ذلك.
- [ ] نقل `index/calendar` إلى Browse Controller متخصص بعد نجاح الاختبارات، ثم تحديث route action فقط دون تغيير route contract.
- [ ] استخراج trash query وController بعد browse.
