...
- A monitoring report owns its verification rows through a cascading FK. Conversely, polymorphic `subject_id` has no FK; permanent Event deletion must transactionally clean generalized `event_target_group`, `execution_teams`, `subject_volunteer_requirements`, generalized `monthly_activity_supplies`, and `monitoring_reports` in a future write/delete layer.
