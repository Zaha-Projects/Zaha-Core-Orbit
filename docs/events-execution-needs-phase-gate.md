...
Execution Needs must remain untouched until the later Monthly Activities data-migration/normalization phase establishes canonical codes and a compatibility mapping. Phase 1.2 closes after the existing shared Event Status and Target Group read slices. No schema, data, config, payload, route, workflow, permission, or morph-map behavior is changed by this decision.
