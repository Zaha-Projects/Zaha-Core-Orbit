                        ...
                        <div class="row g-3">
                            ...
                            <div class="col-12 col-md-4">
                                ...
                                <select class="form-select" name="status" required>
                                    <option value="active" {{ old('status', 'active') === 'active' ? 'selected' : '' }}>{{ __('app.roles.super_admin.users.status.active') }}</option>
                                    <option value="inactive" {{ old('status') === 'inactive' ? 'selected' : '' }}>{{ __('app.roles.super_admin.users.status.inactive') }}</option>
                                </select>
                            </div>
                            <div class="col-12 col-md-4">
                                ...
                                <select class="form-select" name="role" data-user-role-select required>
                                    @foreach ($roles as $role)
                                        <option value="{{ $role->name }}" {{ old('role') === $role->name ? 'selected' : '' }}>{{ $roleLabel($role) }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-12 col-md-6">
                                ...
                                <select class="form-select" name="branch_id">
                                    ...
                                    @foreach ($branches as $branch)
                                        <option value="{{ $branch->id }}" {{ (int) old('branch_id') === (int) $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="col-12 col-md-6">
                            ...
                            <div class="col-12" data-branch-assignment-wrapper hidden>
                                ...
                                <div class="border rounded-3 p-3"><div class="row g-2">
                                    @foreach ($branches as $branch)
                                        <div class="col-12 col-md-6 col-xl-4"><label class="form-check border rounded-3 px-3 py-2 h-100"><input class="form-check-input me-2" type="checkbox" name="assigned_branch_ids[]" value="{{ $branch->id }}" {{ in_array((int) $branch->id, $createAssignedBranchIds, true) ? 'checked' : '' }}><span class="form-check-label">{{ $branch->name }}</span></label></div>
                                    @endforeach
                                </div></div>
                            </div>
                            <div class="col-12">
                    ...
                    <tbody>
                        @forelse ($users as $user)
                            @php
                                ...
                                $primaryRoleName = (string) ($primaryRole?->name ?? '');
                ...
                <div class="modal-dialog modal-xl modal-dialog-scrollable"><div class="modal-content">
                    ...
                    <form method="POST" action="{{ route('role.super_admin.users.update', $user) }}">
                        ...
                        <div class="modal-body"><div class="row g-3">
                            ...
                            <div class="col-12 col-md-4"><label class="form-label">{{ __('app.roles.super_admin.users.fields.status') }}</label><select class="form-select" name="status"><option value="active" {{ $user->status === 'active' ? 'selected' : '' }}>{{ __('app.roles.super_admin.users.status.active') }}</option><option value="inactive" {{ $user->status === 'inactive' ? 'selected' : '' }}>{{ __('app.roles.super_admin.users.status.inactive') }}</option></select></div>
                            <div class="col-12 col-md-4"><label class="form-label">{{ __('app.roles.super_admin.users.fields.role') }}</label><select class="form-select" name="role" data-user-role-select>@foreach($roles as $role)<option value="{{ $role->name }}" {{ $primaryRoleName === $role->name ? 'selected' : '' }}>{{ $roleLabel($role) }}</option>@endforeach</select></div>
                            <div class="col-12 col-md-6"><label class="form-label">{{ __('app.roles.super_admin.users.fields.branch') }}</label><select class="form-select" name="branch_id"><option value="">{{ __('app.roles.super_admin.users.fields.branch_placeholder') }}</option>@foreach($branches as $branch)<option value="{{ $branch->id }}" {{ (int) $user->branch_id === (int) $branch->id ? 'selected' : '' }}>{{ $branch->name }}</option>@endforeach</select></div>
