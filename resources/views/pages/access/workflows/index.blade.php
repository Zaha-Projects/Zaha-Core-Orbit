                ...
                <div class="col-lg-8">
                    ...
                    @foreach($workflow->steps as $step)
                        <div class="border rounded-3 p-3 mb-2">
                            <form method="POST" action="{{ route('role.super_admin.workflow_steps.update', $step) }}" class="wf-grid">@csrf @method('PUT')
                                ...
                                <div class="wf-col-6"><label class="form-label">{{ __('workflow_ui.common.role') }}</label><select class="form-select" name="role_id"><option value="">{{ __('workflow_ui.common.none_option') }}</option>@foreach($roles as $role)<option value="{{ $role->id }}" {{ (int) $step->role_id === (int) $role->id ? 'selected' : '' }}>{{ $role->display_name }}</option>@endforeach</select></div>
                    ...
                    <div class="border rounded-3 p-3 mt-3">
                        ...
                        <form method="POST" action="{{ route('role.super_admin.workflow_steps.store', $workflow) }}" class="wf-grid">@csrf
                            ...
                            <div class="wf-col-6"><label class="form-label">{{ __('workflow_ui.common.role') }}</label><select class="form-select" name="role_id"><option value="">{{ __('workflow_ui.common.none_option') }}</option>@foreach($roles as $role)<option value="{{ $role->id }}">{{ $role->display_name }}</option>@endforeach</select></div>
