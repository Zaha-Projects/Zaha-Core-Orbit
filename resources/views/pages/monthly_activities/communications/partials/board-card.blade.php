<article class="comm-card">
    <div class="comm-card__meta mb-1"><i class="fas fa-location-dot ms-1"></i>{{ $item['branch'] }} • {{ $item['date'] ?: '-' }} • {{ $item['time'] }}</div>
    <h3>{{ $item['title'] }}</h3>
    ...
    <div class="d-flex gap-2 flex-wrap justify-content-between align-items-center"><span class="comm-status-badge">{{ $statusLabels[$item['status']] ?? $item['status'] }}</span><a class="btn btn-sm btn-outline-primary" href="{{ $item['url'] }}">فتح</a></div>
