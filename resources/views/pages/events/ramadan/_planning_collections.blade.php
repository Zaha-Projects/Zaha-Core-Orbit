        ...
        <div class="planning-row border rounded p-3 mb-3">
            ...
            @foreach($fields as $field)<div class="col-md-4"><label class="form-label">{{ __('ramadan_iftars.labels.'.$fieldKeys[$field]) }}</label>
                ...
                @elseif($field === 'has_supporting_entity')
                    <select class="form-select" name="{{ $collection }}[{{ $i }}][{{ $field }}]"><option value="0">{{ __('ramadan_iftars.options.no_supporter') }}</option><option value="1" {{ ($row[$field] ?? false) ? 'selected' : '' }}>{{ __('ramadan_iftars.options.supported') }}</option></select>
                @elseif($field === 'executor_user_id')
                    <select class="form-select" name="{{ $collection }}[{{ $i }}][{{ $field }}]"><option value="">{{ __('ramadan_iftars.options.external_none') }}</option>@foreach($users as $user)<option value="{{ $user->id }}" {{ ($row[$field] ?? null) == $user->id ? 'selected' : '' }}>{{ $user->name }}</option>@endforeach</select>
                @elseif($field === 'beneficiary_segment_id')
                    <select class="form-select" name="{{ $collection }}[{{ $i }}][{{ $field }}]"><option value="">{{ __('ramadan_iftars.options.none') }}</option>@foreach($beneficiarySegments as $segment)<option value="{{ $segment->id }}" {{ ($row[$field] ?? null) == $segment->id ? 'selected' : '' }}>{{ app()->getLocale()==='ar' ? $segment->name_ar : ($segment->name_en ?: $segment->name_ar) }}</option>@endforeach</select>
                @elseif($field === 'gender')
                    <select class="form-select" name="{{ $collection }}[{{ $i }}][{{ $field }}]">@foreach(['male','female','mixed'] as $option)<option value="{{ $option }}" {{ ($row[$field] ?? null) === $option ? 'selected' : '' }}>{{ __('ramadan_iftars.options.'.$option) }}</option>@endforeach</select>
                @else
