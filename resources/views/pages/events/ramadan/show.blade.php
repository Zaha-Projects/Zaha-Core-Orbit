...
<div class="container py-4">
    ...
    <div class="row g-3">
        ...
        @endforeach
        <div class="col-12"><div class="card"><div class="card-header fw-semibold">{{ __('ramadan_iftars.sections.execution_needs') }}</div><div class="table-responsive"><table class="table table-sm mb-0"><thead><tr><th>{{ __('ramadan_iftars.fields.title') }}</th><th>{{ __('ramadan_iftars.fields.expected') }}</th><th>{{ __('ramadan_iftars.fields.actual') }}</th><th>{{ __('ramadan_iftars.fields.execution_status') }}</th></tr></thead><tbody>@forelse($ramadanIftar->executionNeeds as $row)<tr><td>{{ optional($row->executionNeedType)->name }} <span class="badge bg-light text-dark">{{ __('ramadan_iftars.statuses.requirement.'.($row->is_required ? 'required' : 'not_required')) }}</span></td><td>{{ $row->planned_details ?: '—' }}
@if(optional($row->executionNeedType)->code === 'execution_team')<div class="small mt-1">{{ $ramadanIftar->executionTeams->pluck('name')->join('، ') }}</div>@endif
@if(optional($row->executionNeedType)->code === 'supplies')<div class="small mt-1">{{ $ramadanIftar->supplies->pluck('item_name')->join('، ') }}</div>@endif
@if(optional($row->executionNeedType)->code === 'gifts_shields')<div class="small mt-1">{{ $ramadanIftar->gifts->pluck('description')->join('، ') }}</div>@endif</td><td>{{ $row->actual_details ?: '—' }}</td><td>{{ __('ramadan_iftars.statuses.execution.'.$row->status) }}</td></tr>@empty<tr><td colspan="4" class="text-center text-muted">{{ __('ramadan_iftars.empty.execution_needs') }}</td></tr>@endforelse</tbody></table></div></div></div>
