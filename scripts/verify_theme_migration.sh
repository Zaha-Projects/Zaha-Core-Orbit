...
# 2) Role pages should use the unified app layout.
role_pages_count=$(find resources/views/roles -type f -name '*.blade.php' ! -path '*/partials/*' | wc -l | tr -d ' ')
role_app_layout_count=$(rg -n "@extends\('layouts\.app'\)" resources/views/roles -g '*.blade.php' | wc -l | tr -d ' ')
if [[ "$role_app_layout_count" -lt "$role_pages_count" ]]; then
  fail "Not all role pages use layouts.app ($role_app_layout_count/$role_pages_count)"
else
  pass "All role pages use layouts.app ($role_app_layout_count/$role_pages_count)"
fi

# 3) Layout should not contain hardcoded locale/theme labels.
if rg -n "\>\s*(English|العربية|Dark mode|Light mode)\s*\<" resources/views/layouts/app.blade.php >/dev/null; then
  fail "Found hardcoded locale/theme labels in layouts.app"
else
...
for locale in ar en; do
  file="public/assets/theme/locales/$locale/common.json"
