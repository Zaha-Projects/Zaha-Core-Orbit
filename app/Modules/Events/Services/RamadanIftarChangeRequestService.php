...
class RamadanIftarChangeRequestService
{
    ...
    private function copyPlanning(RamadanIftar $source, User $actor): RamadanIftar
    {
        ...
        foreach ($source->supplies as $row) $revision->supplies()->create(array_merge(['subject_type' => EventSubjectTypes::RAMADAN_IFTAR], Arr::only($row->getAttributes(), ['item_name','planned_quantity','provider_type','provider_name','estimated_value','notes']), ['status' => SubjectSupply::STATUS_PENDING]));
