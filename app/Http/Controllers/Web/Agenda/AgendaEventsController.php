...
class AgendaEventsController extends Controller
{
    ...
    public function store(Request $request, ConflictDetectionService $conflicts, WorkflowNotificationService $workflowNotifications)
    {
        ...
        $data = $request->validate([
            ...
            'monthly_plan_template' => ['array', Rule::requiredIf(fn () => $request->input('plan_type') === 'unified')],
            'monthly_plan_template.title' => ['nullable', 'string', 'max:255', Rule::requiredIf(fn () => $request->input('plan_type') === 'unified')],
            'monthly_plan_template.proposed_date' => ['nullable', 'date', Rule::requiredIf(fn () => $request->input('plan_type') === 'unified')],
            'monthly_plan_template.description' => ['nullable', 'string', Rule::requiredIf(fn () => $request->input('plan_type') === 'unified')],
            'monthly_plan_template.responsible_party' => ['nullable', 'string', 'max:255', Rule::requiredIf(fn () => $request->input('plan_type') === 'unified')],
            'monthly_plan_template.target_group' => ['nullable', 'string', 'max:255', Rule::requiredIf(fn () => $request->input('plan_type') === 'unified')],
