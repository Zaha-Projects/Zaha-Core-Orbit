...
class AgendaEventsController extends Controller
{
    ...
    protected function unifiedTemplatePayload(array $data): array
    {
        return [
            ...
            'target_group' => collect(explode(',', (string) ($data['monthly_template_target_group_ids'] ?? '')))->map(fn ($id) => (int) trim($id))->filter()->first(),
            'target_group_ids' => $data['monthly_template_target_group_ids'] ?? null,
    ...
    )
    {
        ...
        $data = $request->validate([
            ...
            'monthly_template_target_group_ids' => ['nullable', 'string'],
        ...
        if (($data['plan_type'] ?? null) === 'unified') {
            validator($data, [
                ...
                'monthly_template_target_group_ids' => ['required', 'string'],
